...
class Playbook {
    ...
    static render(parser) {
        ...
        tasks = {...tasks, ...tasksFromNodeList(nodeList, 1, parser)["tasks"]};
        p.r["tasks"] = tasks;
        return p;
    }
...
function conditionalFromConditionNode(conditionNode, index, parser) {
    ...
    Object.keys(conditionNode.conditions).forEach(conditionText => {
        ...
        task["nexttasks"][conditionText] = newIndex;
        ...
        let taskResult = tasksFromNodeList(nodeList, nextTask);
    ...
    let taskResult = tasksFromNodeList(nextNodeList, nextTask);
    let newTasks = taskResult["tasks"];
    newIndex = taskResult["newIndex"];
    tasks = {...tasks, ...newTasks};
    task["nexttasks"]["else"] = newIndex;

    return {"tasks": tasks, "newIndex": newIndex};
